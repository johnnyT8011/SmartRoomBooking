...
public class BookingScheduler {
    ...
    /** [Refactor #5 Literal constants] Housekeeping sweep interval (was the magic number 60_000). */
    static final long SCAN_INTERVAL_MS = 60_000;
    ...
    public void releaseExpiredLocks() {
        ...
        // [Refactor #4 Hide Delegate] booking.getUserId() / booking.getRoomName() instead of chains.
        for (Booking booking : expired) {
            booking.setStatus(BookingStatus.EXPIRED);
            notificationService.sendSseEvent(booking.getUserId(),
                    "預約鎖逾時未確認，" + booking.getRoomName() + " 時段已自動釋放");
        }
        if (!expired.isEmpty()) {
